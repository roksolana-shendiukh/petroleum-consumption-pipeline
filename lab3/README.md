...
- This deviates from the "pure" bronze principle (raw as-is), but is justified for a streaming source backed by an open global firehose – filtering at the system boundary is a standard approach when paying to transmit unneeded data makes no sense
...
- Authentication uses a SAS connection string (via Databricks secret) instead of AAD/service principal – simpler for a lab, but production would use AAD auth for centralized RBAC and credential rotation instead of a long-lived shared key
